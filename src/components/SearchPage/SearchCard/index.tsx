...
export const SearchCard: React.FC<SearchResult> = (props) => {
  ...
  return (
    <div
      className={`flex w-full flex-col p-6 md:flex-row ${props.image_url ? 'gap-8' : ''} rounded-lg border border-[#4DA6E6] bg-white shadow-sm lg:max-h-64`}
    >
      {props.image_url && (
        <div className="relative w-40 h-40 lg:h-full justify-center">
          <Image src={props.image_url} alt={props.title} fill className="object-cover rounded-lg" />
        </div>
      )}
